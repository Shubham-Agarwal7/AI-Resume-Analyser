...
connection = pymysql.connect(host='localhost',user='root',password='Yash1234',db='AI_Resume')
...
def run():
    ...
    img = Image.open('D:/Udemy/ML/Models/Courses/Ai_resume.jpg')
    img = img.resize((250,250))
    st.image(img)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS AI_Resume;"""
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            #     time.sleep(4)
            save_image_path = 'D:/Udemy/ML/Models/Uploaded_Resumes/'+pdf_file.name
            ...
            if resume_data:
                ...
                if 'Achievements' or 'Experience' in resume_text:
                    resume_score = resume_score + 20
                ...
                if 'Project' or 'Projects' in resume_text:
                    resume_score = resume_score + 20
                ...
                for i in resume_videos:
                    st.markdown(f'<iframe src={i} width="420" height="315"> </iframe>', unsafe_allow_html=True)
        ...
        if st.button('Login'):
            if ad_user == 'ai_resumer' and ad_password == 'yash123':
